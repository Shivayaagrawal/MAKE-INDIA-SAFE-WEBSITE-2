...
export interface WaveGridBackgroundProps {
  ...
  className?: string
  gridSize?: number
  /** Tile footprint width/depth. Smaller = finer grid. */
  cubeWidth?: number
  /** Tile height (extrusion). */
  cubeHeight?: number
  colorBase?: string
  colorHigh?: string
  ...
  waveJitter?: number
  autoAnimate?: boolean
  /** Seconds of idle time before auto-ripples resume. Defaults to 5. */
  autoIdleDelay?: number
  /** Seconds between auto-ripples when idle. Defaults to 5.5. */
...
export function WaveGridBackground({
  ...
  useEffect(() => {
    ...
    const updateTrail = (delta: number) => {
      ...
      if (
        timeSinceLastMove >= propsRef.current.autoIdleDelay &&
        !placingRandom &&
        propsRef.current.autoAnimate
      ) {
        placingRandom = true
        randomPointTimer = 0
      }
      if (placingRandom && propsRef.current.autoAnimate) {
        ...
        if (randomPointTimer >= propsRef.current.autoRippleInterval) {
          addRandomPoint()
          randomPointTimer = 0
        }
      }
    ...
    // Scene is built once; live values flow through propsRef.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [gridSize, cubeWidth, cubeHeight, vignette])

  return (
    <div
      ref={containerRef}
      className={cn('wave-grid-background', className)}
    >
      <canvas ref={canvasRef} className="wave-grid-background__canvas" />
      {children != null && (
        <div className="wave-grid-background__content">{children}</div>
      )}
    </div>
  )
}
