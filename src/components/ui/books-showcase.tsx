...
export function BooksShowcase({
  ...
  useEffect(() => {
    ...
    function computeSlots() {
      ...
      const fit = portrait ? clamp(a / 1.25, 0.32, 0.58) : clamp(a / 2.05, 0.42, 0.78);
      ...
      if (SLOTS.portrait) {
        ...
        const spread = clamp((a - 1.45) * 0.85, 0, 1.25);
        SLOTS.detail = {
          p: [-1.68 - spread, 0.0, 0.85],
          r: [0.02, -0.44, 0.08],
          s: 1.06,
        };
      }
    ...
    >
      ...
      {showDetailPanel && (
        <div
          ref={dpRef}
          aria-live="polite"
          className={`absolute right-[6%] top-1/2 z-[15] w-[min(620px,40%)] -translate-y-1/2 pointer-events-none @max-[760px]:right-auto @max-[760px]:left-1/2 @max-[760px]:top-auto @max-[760px]:bottom-[3.5%] @max-[760px]:w-[min(560px,92cqw)] @max-[760px]:-translate-x-1/2 @max-[760px]:translate-y-0 ${panelVisible ? 'visible' : 'invisible delay-[500ms]'
            }`}
        >
          ...
          <p className={`mt-[22px] text-[15px] italic text-[var(--bs-peri)] ${dpChild(210)}`}>
            An everyday signal — not a diagnosis.
          </p>
          <div className={`mt-[26px] border-t border-[var(--bs-lav)]/[0.18] @max-[760px]:mt-4 ${dpChild(270)}`} />
          <p className={`mt-6 max-w-[46ch] text-[15px] leading-relaxed text-[var(--bs-lav)] ${dpChild(330)}`}>
            Gut health often sits underneath complaints like this. Understanding the connection is the first useful step.
          </p>
        </div>
      )}
    </div>
